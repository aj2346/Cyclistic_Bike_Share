...
##Average length of rides per month (member vs. casual)
  ...
  bike_rides3 %>%  #Average ride length by customer type and month
  group_by(member_casual, year_month) %>% 
...
![image](https://user-images.githubusercontent.com/96959280/154797088-bd4900cc-9b59-4f7a-8686-e8a530427651.png)
 ...
 ##Average length of rides per week (member vs. casual)
  ...
  bike_rides3 %>%   #Average length by Customer Type and Day of Week
  group_by(member_casual, weekday) %>% 
  summarise(average_ride_length = mean(ride_time_m)) %>% 
...
```
 ...
 IL <- States[States$NAME_1 == "Illinois",]
 plot(IL)
 set.seed(12345)
 randPts <- sp::spsample(x = IL, n = 100, type = "random")
 plot(IL)
 plot(randPts, 
     add = TRUE,
     pch = 19)
...
Narrow1 <- maptools::elide(arrow1, shift = c(extent(IL)[2],extent(IL)[3]))
plot(Narrow1, add = TRUE,col = "black")
arrow2 <- layout.north.arrow(type = 2)
Narrow2 <- maptools::elide(arrow2, shift = c(extent(IL)[1]-0.5,extent(IL)[3]))
plot(Narrow2, add = TRUE, col = "blue")
raster::scalebar(d = 100, # distance in km
                 xy = c(extent(IL)[1]+0.01,extent(IL)[3]+0.15),
                 type = "bar", 
                 divs = 2, 
                 below = "km", 
                 lonlat = TRUE,
...
NH <- States[States$NAME_1 == "Illinois",]

# This call makes the map
  # inital map
mapIL <- leaflet(data = IL) %>%  
  # add a baselayer
      addTiles() %>% 
...
library(ggplot2)
library(viridis)
IL <- st_read(system.file("shape/il.shp", package = "sf"),
  quiet = TRUE
)
ggplot(data = il, aes(fill = SID74)) + geom_sf() +
  scale_fill_viridis() + theme_bw()
library(geoR)
ggplot(data.frame(cbind(bike_rides3$coords, stations = start_lat$start_lng)))+
  geom_point(aes(east, north, color = Rainfall), size = 2) +
  coord_fixed(ratio = 1) +
  scale_color_gradient(low = "blue", high = "orange") +
  geom_path(data = data.frame(bike_rides3$border), aes(east, north)) +
  theme_bw()
...
start_loc <- unique(bike_rides3[,c('start_lng', 'start_lat', 'start_station_id', 'start_station_name')]) %>% rename(Longitude = start_lng, Latitude = start_lat, Stid = start_station_id, Stname = start_station_name)
    
end_loc <- unique(bike_rides3[,c('end_lng', 'end_lat', 'end_station_id', 'end_station_name')]) %>% rename(Longitude = end_lng, Latitude = end_lat, Stid = end_station_id, Stname = end_station_name) 
